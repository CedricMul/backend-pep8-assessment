...
key = KEY_RIGHT
score = 0
# Initial snake and food co-ordinates,
# prints the food
snake = [[4, 10], [4, 9], [4, 8]]
food = [10, 20]
...
while key != 27:    # While Esc key is not pressed
    ...
    if key == ord(' '):
        ...
        key = prevKey
        continue
    # If an invalid key is pressed, keep previous key
    if key not in [
            KEY_LEFT, KEY_RIGHT,
            KEY_UP, KEY_DOWN, 27]:
        key = prevKey
    # Calculates the new coordinates of the head of the snake.
    # NOTE: len(snake) increases. This is taken care of later at [1].
    snake.insert(
                0,
                [
                    snake[0][0]
    ...
    # If the snake crosses the boundaries,
    ...
    if snake[0][1] == 59:
        snake[0][1] = 1
    # Option to exit if the snake
    # crosses the boundaries below (Uncomment to enable)
    # if snake[0][0] == 0 or snake[0][0] == 19 \
    # or snake[0][1] == 0 \
    # or snake[0][1] == 59:
    # break
    # If snake runs over itself, exit
    ...
    if snake[0] == food:   # When snake eats the food
        ...
        while food == []:
            ...
            if food in snake:
                food = []
        win.addch(food[0], food[1], '*')
    else:    # [1] If it does not eat the food, length decreases
        last = snake.pop()
